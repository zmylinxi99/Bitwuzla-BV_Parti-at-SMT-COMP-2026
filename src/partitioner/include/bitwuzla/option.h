 ...
 */
  ...
  /*!
   * **Preprocessing: Feasible domain propagation.**
   *
   * When enabled, applies the feasible-domain propagation preprocessing pass.
   *
   * *Values:*
   *  - **true**: enable [**default**]
   *  - **false**: disable
   */
  EVALUE(PP_FDP_ENABLE),
  /*!
   * **Preprocessing: Feasible domain export prefix.**
   *
   * If non-empty, assertions after feasible-domain propagation are dumped to
   * `<prefix>/task-<id>-simplified.smt2`. When `#PP_FDP_NEXT_ID` is non-zero,
   * two split tasks are also emitted as
   * `<prefix>/task-<next-id>.smt2` and `<prefix>/task-<next-id+1>.smt2`.
   *
   * *Values:*
   *  - A string denoting the directory prefix to write to. [**default:** ""]
   */
  EVALUE(PP_FDP_DUMP_PREFIX),
  /*!
   * **Preprocessing: Feasible domain current node id.**
   *
   * Used to align FDP dumping/splitting with STP's partition ids. Simplified
   * output is tagged with this id.
   *
   * *Values:*
   *  - A numeric id for the current node. [**default:** 0]
   */
  EVALUE(PP_FDP_NODE_ID),
  /*!
   * **Preprocessing: Feasible domain next node id.**
   *
   * If non-zero and `#PP_FDP_DUMP_PREFIX` is set, left split task uses this id,
   * right split task uses this id + 1.
   *
   * *Values:*
   *  - A numeric id for the left child task. [**default:** 0]
   */
  EVALUE(PP_FDP_NEXT_ID),
  /*!
   * **Preprocessing: Feasible domain partition seed.**
   *
   * If set, this seed is used for FDP partitioning and overrides
   * `#PP_FDP_NODE_ID`.
   *
   * *Values:*
   *  - A numeric seed for partitioning. [**default:** 0]
   */
  EVALUE(PP_FDP_PARTI_SEED),
