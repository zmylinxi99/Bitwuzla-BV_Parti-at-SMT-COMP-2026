...
Preprocessor::Preprocessor(SolvingContext& context)
    : d_env(context.env()),
      ...
      d_pass_feasible_domain(d_env, &d_backtrack_mgr),
...
Preprocessor::preprocess()
{
  ...
  d_pass_feasible_domain.clear_cache();
...
Preprocessor::apply(AssertionVector& assertions)
{
  ...
  // #linxi: feasible domain propagation
  pp_apply_loop();

  // // if (!options.pp_fdp_dump_prefix().empty()) { // pass the substitutions
  // to fdp pass
  //     d_pass_feasible_domain.set_substitutions(&substitutions());
  // // }

  if (options.pp_fdp_enable())
  {
    // if (false && options.pp_fdp_enable()) {
    if (!assertions.is_inconsistent())
    {
      d_pass_feasible_domain.apply(assertions);
      // exit(0);
      if (d_logger.is_msg_enabled(1))
      {
        print_statistics(d_pass_feasible_domain, assertions);
      }
      if (!assertions.is_inconsistent()
          && !d_pass_feasible_domain.has_rolled_back())
      {
        // re-apply fixed-point passes after feasible domain propagation
        pp_apply_loop();
      }
    }
  }
