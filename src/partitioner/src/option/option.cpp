...
Options::Options()
    : d_diagnostic_output_stream(&std::cerr),
      ...
      // Feasible Domain Propagation (FDP) dumping options
      pp_fdp_enable(this,
                    Option::PP_FDP_ENABLE,
                    true,
                    "enable feasible domain propagation preprocessing pass",
                    "pp-fdp-enable"),

      pp_fdp_dump_prefix(
          this,
          Option::PP_FDP_DUMP_PREFIX,
          "",
          "write assertions after feasible domain propagation to "
          "<prefix>/task-<id>-simplified.smt2 (empty disables dumping); "
          "set pp-fdp-next-id to emit child tasks",
          "pp-fdp-dump-prefix"),
      pp_fdp_node_id(this,
                     Option::PP_FDP_NODE_ID,
                     0,
                     0,
                     UINT64_MAX,
                     "current partition node id for FDP dumping",
                     "pp-fdp-node-id"),
      pp_fdp_next_id(
          this,
          Option::PP_FDP_NEXT_ID,
          0,
          0,
          UINT64_MAX,
          "next partition node id (left child); right child uses next-id+1",
          "pp-fdp-next-id"),
      pp_fdp_parti_seed(
          this,
          Option::PP_FDP_PARTI_SEED,
          0,
          0,
          UINT64_MAX,
          "seed for FDP partitioning (overrides pp-fdp-node-id when set)",
          "pp-fdp-parti-seed"),
...
Options::data(Option opt)
{
  switch (opt)
  {
    ...
    case Option::PP_FDP_ENABLE: return &pp_fdp_enable;
    case Option::PP_FDP_DUMP_PREFIX: return &pp_fdp_dump_prefix;
    case Option::PP_FDP_NODE_ID: return &pp_fdp_node_id;
    case Option::PP_FDP_NEXT_ID: return &pp_fdp_next_id;
    case Option::PP_FDP_PARTI_SEED: return &pp_fdp_parti_seed;
